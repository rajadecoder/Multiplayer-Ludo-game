...
/////function to create levels
  $("th").click(function(){
    topic=this.id;
     head =  $(this).attr('class');
  //   topic_levels=english_data[db][main_topic][topic].sub.length; ///taking the number of levels from each topic when clicked


    localStorage.setItem("topic",topic);
    localStorage.setItem("main_top",main_top);
    localStorage.setItem("head",head);
    localStorage.setItem("db",db);
    localStorage.setItem("topic_levels",topic_levels);
    localStorage.setItem("main_topic",main_topic);
window.location="level_page.html";
...
window.location="question.html";
  ...
  //document.getElementById("na").removeAttribute("onclick","goBack()");
    //  $("#na").attr("onclick","goBack2()");

}

function m_topic(x,y){
